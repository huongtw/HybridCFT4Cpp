...
import HybridAutoTestGen.BoundedTestGen;
import compiler.AvailableCompiler;
...
//        executor.execute();
    ...
    protected void btnHybrid_Clicked(ActionEvent event) throws Exception
    {
      ...
      //  double boundStep = 0.5; //voi so thuc
        double boundStep = 1; //voi so nguyen
    ...
    @FXML
    protected void btnBVTGKSE_Clicked(ActionEvent event) throws Exception
    {
...
//                bGen.boundaryValueTestGen();
                BoundedTestGen bounedGen = new BoundedTestGen(maxloop, value, txtSourceFolder.getText());
                bounedGen.analyze();
                VNUDSE.getStage().setTitle("Running test cases and calculating coverage...");

//                FunctionCoverageComputation functionCoverageComputation = Utils.ExecuteTestCase(txtSourceFolder.getText(), value, bounedGen.getTestCases());
//
//                bounedGen.setFunctionCoverageComputation(functionCoverageComputation);
//
                VNUDSE.getStage().setTitle("Exporting test report...");
//
                bounedGen.ExportReport();
//
                VNUDSE.getStage().setTitle(TestConfig.TOOL_TITLE);
